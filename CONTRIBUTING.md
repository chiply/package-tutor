...
package-tutor depends only on built-in Emacs libraries, so there are no
runtime package dependencies to install.

## Setup

```sh
git clone https://github.com/chiply/package-tutor.git
cd package-tutor
...
```sh
eask test ert test/package-tutor-test.el
```
...
- Prefix all public symbols with `package-tutor-` and private symbols
  with `package-tutor--`.
...
later**. By submitting a contribution you agree that your work will be
